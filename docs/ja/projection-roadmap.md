...
初期profileでは、MST wallの22,704 triangleを逐次走査し、triangleごとにCSR行と少数点の
intersectionを処理するPython overheadが小規模caseを支配した。coreの幾何判定は変更せず、
point/triangleのbounded batch内でcone候補とMöller--Trumbore判定を消費し、遮蔽済みvertexを
後続triangle batchから除外する実装を検証中である。制限MST `shape=(24,16,12)` ではreferenceと
visibility hash・voxel状態数が一致し、cold preflightはローカル測定で7.14 sから0.130 sへ短縮した。
これは開発用small-grid結果であり、d=10の受け入れ値ではない。
同じ処理はprojection構築中のpartial subvoxel visibilityにも使われる。制限MST
`shape=(8,6,4)`, `partial_res=2`, single cameraの初期比較ではPの差分nnzは0で、cache-hit後の
projection構築は1.13 sから0.026 sへ短縮した。partialを含む大きなcaseでの受け入れは未完了である。

full bounding d=10も実行した。中心range `(-2015,2015)`, `(-2015,2015)`, `(-515,515)`、
shape `(404,404,104)`、16,974,464 voxel、inside vertex 7,394,064のtwo-camera caseで、
allocation追跡なしのcold preflightは305.0 s、process peak RSSは約2.57 GB、cache-hitは0.48 sだった。
各cameraで約96.5%のvoxelが不可視だった。wall判定だけで左右146.5 sと143.5 sを占め、
