...
fully visible voxelの軸別adaptive判定は実装済みだが、これは幾何学的なsampling指標であり、
profile画像の誤差toleranceではない。fixed `res`を基準・高精度経路、adaptiveを高速化optionとして
扱う。代表的なvisible voxelについて `res=1,2,4,...` の投影結果を比較する収束確認は、defaultの
自動判定には組み込まず、必要になった場合だけ高コストな診断modeとして追加する。

- fixed `res`経路は変更せず、高精度計算とadaptive検証のreferenceとして残す。
- adaptive判定はfully visible voxelだけに適用し、partial voxelには`partial_res`を使う。
- 判定に使った無次元比、採用res、上限到達の有無を診断情報として返す。
- wallやinside境界の不連続性、任意profileに対する画像誤差は幾何判定の保証対象にしない。
...
#### 外接球による単純なres=1足切りへの再設計候補

現在は各voxelについてworld 3軸の両端、合計6点をscreenへ実投影して軸別resを決める。8 cornerを
使う実装ではないが、`N` voxelに対して `6N` 点を生成・座標変換・投影するため、`d=10 mm`の
約1720万voxelではadaptive判定だけで約1億点を扱うことになる。点光源近似による計算削減が目的
であることを考えると、この前処理コストと厳密な端点投影のgainは釣り合わない可能性が高い。

次の候補では、voxelに外接する球の直径
...
と評価する。`1/cos(theta)`は斜視時のworst-caseを含めるためのもので、voxel cornerの実投影は
行わない。これを
...
で無次元化する。投影方向を事前に特定しないscalar worst-caseなので、screen 2軸のうち小さい
reference scaleを使う。`rho`が単一の保守的な閾値以下なら `(1, 1, 1)`を採用する。閾値を小さくすれば
細かいres側へ倒れる。pixel境界、profile勾配、距離帯ごとの補正など、個別のヒューリスティックは
追加しない。判定を通らないvoxelは、ユーザーが指定したfixed上限resを使う。このためadaptiveの
意味は「明らかに点光源とみなせるfully visible voxelだけres=1へ落とす」と限定される。

scalar `res=r`は最長辺方向の分割数と解釈し、voxelの縦横比から立方体に近いsubvoxelを作る
axis-wise wrapperを用意する。
...
従来どおり `(r, r, r)`になる。明示的なtuple `res=(r_x,r_y,r_z)`はそのまま使用し、上級者が
軸別値を直接指定できるようにする。partial voxelはこの足切りの対象にせず、引き続き
`partial_res`を明示する。

この変更を実装する前に、現行6端点方式との比較をToyとMST短時間モデルで行う。最低限、
res=1と判定されるvoxel集合、展開sample数、projection/profile誤差、判定時間を比較する。
外接球方式は現行より保守的でよく、目的は中間resの最適化ではなく、fine gridにおいて判定時間を
`O(6N)`点投影から中心点だけの安価なベクトル演算へ落とすことである。

なお、voxel格子が実際のemissionを表現できない誤差と、表現済みの三線形補間profile `S f`を
有限個のsubvoxel点で積分する誤差は区別する。source resを上げて改善できるのは後者だけであり、
前者にはvoxel grid自体の細分化が必要である。
