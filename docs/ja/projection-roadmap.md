...
`psf_tolerance`、`psf_metric`、`psf_grouping`、`max_factorized_byte_fraction`である。work chunkでは
複数scopeのsubvoxel点をまとめて1回のPSF計算に渡す一方、groupingはscopeごとに独立に行う。
...
現在のproduction実装では、scopeごとにexact direct `P_c=I_c S_c`とfactorized候補`Q_c,A_c`を
構築し、CSRの実payload Bytesを比較する。
`B(Q_c)+B(A_c) <= max_factorized_byte_fraction * B(P_c)`の場合だけfactorized表現を採用する。
defaultは`max_factorized_byte_fraction=0.8`で、20%以上の削減を要求する。`0`はgroupingをせず
常にdirect、`None`は非空factorizationを常に保持する。`N_group/N_active_sample`は圧縮構造の診断値
として残すが採否には使わない。統計には選択後Bytesに加え、candidate direct/Q/AのBytesを保持する。
...
`Q+A`のstorageを判定できず、旧`max_group_fraction=0.8`は採否基準として緩すぎる。
...
この結果とMST評価を踏まえ、scopeごとのexact direct Bytes比較をproductionへ導入した。
...
追跡でき、pixel空間化でprojection cache schemaを2へ上げた。さらにcompression statsの候補Bytes
表現追加でschemaを3へ上げたため、旧pickleのvisibilityは残してprojectionだけを再計算する。
...
この旧閾値・scope採用基準ではまだ約5.3%不利であり、exact Bytes採否を導入する根拠となった。
...
source res 2、旧`max_group_fraction=0.8`では、detector res 1/5の保存量とgroup数はそれぞれ
...
exact Bytes採否の初回実装では、scopeごとのCSR payloadだけを比較しても最終保存量を保証できない
ことが分かった。各scopeのdirect CSRが持つpixel-row `indptr`は最終結合時に共有され、隣接scopeの
重複要素も加算・統合されるため、scope Bytesの和と最終`P` Bytesは一致しない。そこでscope採否に
加えて、work chunkごとにexact direct blockを一時構築し、eye全体を結合した後に最終hybrid Bytesと
exact `P` Bytesを再比較する二段階gateを追加した。最終条件を満たさなければeye全体をdirectへ戻す。

このgateをpixel空間Toy（144 voxel、source res 2、detector res 5、閾値0.1）に適用すると、
`Z/f=5/15/50`の全条件でfactorized scopeは最終的に0となり、画像誤差は`1e-16`程度へ戻った。
MST `d=75 mm`でも`0/1251 scope`で、構築`14.60 s`、保存`8,817,752 bytes`だった。通常Pの
`8,802,860 bytes`との差0.17%はdirect-only `HybridProjectionOperator`の空`Q/A` CSR管理領域である。
...
2. ~~`d=75 mm` MST 数値検証と detector res の決定~~（pixel空間group化後を再測定済み）
3. inside 境界仕様の確定
4. main への統合
...
11. ~~fine `P`と構築時間・peak memory・CSR実Bytes・forward/backproject時間を比較し、Bytes gateを導入~~
12. 不採用grouping用cheap prefilterと、tolerance/bin幅を変えた圧縮可能領域を評価
13. ~~圧縮が不利な近距離scopeを非圧縮blockとして混在させるoperator表現を追加~~
14. compact mappingとgroup化だけでは不足する場合に adaptive voxel mergingを検討
15. PD array、任意 Eye 形状、Eye 内部位置ごとの局所visibilityの必要性を定量化
